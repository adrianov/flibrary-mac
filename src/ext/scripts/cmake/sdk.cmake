...
elseif(APPLE)
	set(LIB_INSTALL_DESTINATION ../Frameworks)
	set(LIB_DESTINATION ${CMAKE_BINARY_DIR}/lib)
endif()
...
function(CopyAndInstallQtModules)
	...
	foreach(lib ${ARGN})
		...
		if (NOT WIN32)
			...
			if(APPLE)
				list(APPEND QT_BIN_FILES "${lib_path}/${lib_name}.framework/Versions/A/${lib_name}")
			else()
...
function(CopyAndInstallQtPlugins)
	foreach(plugin ${ARGN})
		...
		if (NOT TARGET ${plugin_target})
			...
			if(APPLE)
				set(plugin_dest "${CMAKE_BINARY_DIR}/bin/${PROJECT_NAME}.app/Contents/PlugIns/${plugin_type}")
				set(plugin_install_dest "../PlugIns/${plugin_type}")
			else()
				set(plugin_dest "${CMAKE_BINARY_DIR}/bin/${plugin_type}")
				set(plugin_install_dest "${plugin_type}")
			endif()
			file(COPY ${plugin_location} DESTINATION "${plugin_dest}")
			install(FILES ${plugin_location} DESTINATION "${plugin_install_dest}")
...
function(CopyAndInstallICU)
	...
	set(ICU_BIN_FILES)
	if(NOT WIN32)
		list(APPEND ICU_BIN_FILES ${icu_RES_DIRS_${CMAKE_BUILD_TYPE_UPPER}}/icudt${ICU_MAJOR_VERSION}l.dat)
	endif()
	...
	foreach(lib ${LIBS})
		...
		elseif(APPLE)
			list(APPEND ICU_BIN_FILES ${ICU_LIB_DIR}/libicu${lib}.${ICU_VERSION_STRING}.dylib)
			list(APPEND ICU_BIN_FILES ${ICU_LIB_DIR}/libicu${lib}.${ICU_MAJOR_VERSION}.dylib)
		else()
...
function(CopyAndInstallDjVuLibre)
	...
	if(WIN32)
		...
		list(APPEND DJVU_BIN_FILES "${LIB_DESTINATION}/djvulibre.dll")
	elseif(APPLE)
		set(_djvu_lib_dir "${djvulibre_LIB_DIRS_${CMAKE_BUILD_TYPE_UPPER}}")
		file(GLOB _djvu_libs "${_djvu_lib_dir}/libdjvulibre*.dylib")
		foreach(_djvu_lib ${_djvu_libs})
			get_filename_component(_djvu_name ${_djvu_lib} NAME)
			if(IS_SYMLINK "${_djvu_lib}")
				continue()
			endif()
			configure_file("${_djvu_lib}" "${LIB_DESTINATION}/${_djvu_name}" COPYONLY)
			list(APPEND DJVU_BIN_FILES "${LIB_DESTINATION}/${_djvu_name}")
		endforeach()
		foreach(_djvu_lib ${_djvu_libs})
			get_filename_component(_djvu_name ${_djvu_lib} NAME)
			if(NOT IS_SYMLINK "${_djvu_lib}")
				continue()
			endif()
			file(READ_SYMLINK "${_djvu_lib}" _djvu_link_target)
			file(CREATE_LINK "${_djvu_link_target}" "${LIB_DESTINATION}/${_djvu_name}" SYMBOLIC)
			list(APPEND DJVU_BIN_FILES "${LIB_DESTINATION}/${_djvu_name}")
		endforeach()
	else()
		set(BIN_FILES libdjvulibre.so libdjvulibre.so.21 libdjvulibre.so.21.7.0)
		foreach(bin_file ${BIN_FILES})
			file(COPY "${djvulibre_LIB_DIRS_${CMAKE_BUILD_TYPE_UPPER}}/${bin_file}" DESTINATION ${LIB_DESTINATION})
			list(APPEND DJVU_BIN_FILES "${LIB_DESTINATION}/${bin_file}")
		endforeach()
	endif()
	install(FILES ${DJVU_BIN_FILES} DESTINATION ${LIB_INSTALL_DESTINATION})	
endfunction()
