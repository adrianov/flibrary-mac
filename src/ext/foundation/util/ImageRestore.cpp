...
QByteArray PrepareToExportImpl(QIODevice& stream, const QString& folder, const QString& fileName, const ISettings& settings, std::unique_ptr<const ExtractedBook> metadataReplacement)
{
	...
	const auto storedEpub = fileName.endsWith(QStringLiteral(".epub"), Qt::CaseInsensitive);
	if (covers.empty() && imageProcessing == ImageProcessing::None && !metadataReplacement && !storedEpub)
		return stream.readAll();
...
QByteArray PrepareToExport(QIODevice& input, const QString& folder, const QString& fileName, const ISettings& settings)
{
	return PrepareToExport(input, folder, fileName, settings, nullptr);
}
