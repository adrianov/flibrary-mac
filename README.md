...
1. **Сборка в `.app`** — проект собирается как macOS-пакет (`FLibrary.app`): Qt, ICU, 7-Zip и остальные библиотеки входят в `Contents/Frameworks`, поэтому Homebrew на целевой системе не нужен.
...
`FLibrary.app` несёт зависимости в `Contents/Frameworks` и работает без установленного Homebrew.
