...
1. **Сборка в `.app`** — проект собирается как полноценный пакет macOS (`FLibrary.app`) с Qt, ICU, 7-Zip и остальными библиотеками внутри `Contents/Frameworks`. На машине без Homebrew приложение запускается как есть.
...
4. **Открытие `.fb2` из Finder** — FLibrary зарегистрирован как обработчик FictionBook; двойной щелчок или «Открыть с помощью» конвертирует файл в EPUB (кэш в `~/Library/Caches/…/epub/`) и открывает его в Books.
5. **Ускорение работы** — кэширование на диске (конвертация FB2→EPUB, аннотации, навигация по каталогу, предзагрузка соседних книг в списке) и оптимизация отрисовки UI для плавной прокрутки по большим библиотекам.
6. **Языки интерфейса** — добавлены токипона и эсперанто; переключение языка без перезапуска программы.
7. **Linux и Windows** — поддержка Linux сохранена; сборку под Windows в этой ветке не проверял.
...
Приложение собирается как `FLibrary.app`; зависимости лежат в `Contents/Frameworks`. Готовый пакет не требует Homebrew на целевой машине.
